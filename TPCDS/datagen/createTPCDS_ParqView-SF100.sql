...
create or replace view customer as select 
  ...
  c_email_address, 
  c_last_review_date 
from dfs.`/tpcdsParquet/SF100/customer`;
...
create or replace view customer_address as select 
 ...
 ca_gmt_offset, 
 ca_location_type 
from dfs.`/tpcdsParquet/SF100/customer_address`;
...
from dfs.`/tpcdsParquet/SF100/customer_demographics`;
...
create or replace view household_demographics as select 
 ...
 hd_dep_count, 
 hd_vehicle_count 
from dfs.`/tpcdsParquet/SF100/household_demographics`;
...
create or replace view item as select 
 ...
 i_manager_id, 
 i_product_name 
from dfs.`/tpcdsParquet/SF100/item`;
...
create or replace view promotion as select 
 ...
 p_purpose, 
 p_discount_active 
from dfs.`/tpcdsParquet/SF100/promotion`;
...
create or replace view time_dim as select 
 ...
 t_sub_shift , 
 t_meal_time 
from dfs.`/tpcdsParquet/SF100/time_dim`;
...
create or replace view date_dim as select 
 ...
 d_current_quarter, 
 d_current_year 
from dfs.`/tpcdsParquet/SF100/date_dim`;
...
create or replace view store as select 
 ...
 s_gmt_offset, 
 s_tax_precentage 
from dfs.`/tpcdsParquet/SF100/store`;
...
create or replace view store_sales as select 
 ...
 ss_net_paid_inc_tax, 
 ss_net_profit 
from dfs.`/tpcdsParquet/SF100/store_sales`;
...
create or replace view warehouse as select 
   ...
   w_country,
   w_gmt_offset        
from dfs.`/tpcdsParquet/SF100/warehouse`;
...
create or replace view ship_mode as select 
   ...
   sm_carrier,
   sm_contract      
from dfs.`/tpcdsParquet/SF100/ship_mode`;

create or replace view reason as select 
   r_reason_sk,
   r_reason_id,
   r_reason_desc  
from dfs.`/tpcdsParquet/SF100/reason`;

create or replace view income_band as select 
   ib_income_band_sk,
   ib_lower_bound,
   ib_upper_bound          
from dfs.`/tpcdsParquet/SF100/income_band`;
...
create or replace view call_center as select 
   ...
   cc_gmt_offset,
   cc_tax_percentage  
from dfs.`/tpcdsParquet/SF100/call_center`;
...
create or replace view web_site as select 
   ...
   web_gmt_offset,
   web_tax_percentage 
from dfs.`/tpcdsParquet/SF100/web_site`;
...
create or replace view store_returns as select 
   ...
   sr_store_credit,
   sr_net_loss    
from dfs.`/tpcdsParquet/SF100/store_returns`;
...
create or replace view web_page as select 
   ...
   wp_image_count,
   wp_max_ad_count          
from dfs.`/tpcdsParquet/SF100/web_page`;
...
create or replace view catalog_page as select 
   ...
   cp_description,
   cp_type         
from dfs.`/tpcdsParquet/SF100/catalog_page`;
...
from dfs.`/tpcdsParquet/SF100/inventory`;
...
create or replace view catalog_returns as select 
   ...
   cr_store_credit,
   cr_net_loss        
from dfs.`/tpcdsParquet/SF100/catalog_returns`;
...
create or replace view web_returns as select 
   ...
   wr_account_credit,
   wr_net_loss         
from dfs.`/tpcdsParquet/SF100/web_returns`;
...
create or replace view web_sales as select 
   ...
   ws_net_profit        
from dfs.`/tpcdsParquet/SF100/web_sales`;
...
create or replace view catalog_sales as select 
   ...
   cs_net_profit         
from dfs.`/tpcdsParquet/SF100/catalog_sales`;
